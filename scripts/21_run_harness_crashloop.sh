...
# Run the CrashLoopBackOff harness scenario
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(dirname "$SCRIPT_DIR")"

NAMESPACE="aiops-harness"
JOB_NAME="harness-crashloop-$(date +%Y%m%d%H%M%S)"
CONFIGMAP="harness-manifest-crashloop"

echo "=== AIOps Harness — CrashLoopBackOff Scenario ==="
echo ""
...
oc get deployment ratings-v1 -n bookinfo > /dev/null 2>&1 || {
    echo "ERROR: Bookinfo not deployed. Run ./scripts/10_deploy_all.sh first."
    exit 1
}

# Delete any previous job with same base name
oc delete job -l scenario=crashloop -n "$NAMESPACE" --ignore-not-found 2>/dev/null
