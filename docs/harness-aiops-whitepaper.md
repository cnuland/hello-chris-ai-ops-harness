...
## 10.4 Experiment Tracking with MLFlow

Two MLFlow instances provide experiment tracking at different levels of the architecture, maintaining the External Independence Principle through physical separation:

**AIOps Pipeline Tracker.** Lives on the pipeline side, in the `mlflow-aiops` namespace. Each Llama Stack agent session maps to one MLFlow run. The tracker logs: model ID, scenario, every tool call made (with parameters and results), evidence retrieved, per-tool latency, total investigation time, MTTD (when alert-triggered), and the final RCA output. This enables the AIOps team to compare prompt strategies, RAG configurations, and model versions — understanding *how* the pipeline investigates and where it spends its time.

**Harness Evaluation Tracker.** Lives on the harness side, in the `mlflow-harness` namespace. Each harness run maps to one MLFlow run. The tracker logs: all six scoring dimensions, weighted composite, PASS/FAIL determination, cross-model judge matrix, fact-checking results (which pipeline claims the eval model confirmed versus contradicted), and hallucination flags. This enables regression tracking across model updates, prompt changes, and harness configuration changes — understanding *how well* the pipeline investigated and whether its claims hold up under independent verification.
...
**Summary:** Every harness run produces scoring data that drives model improvement. The maturity model provides four stages from observation to bounded autonomy, each justified by accumulated scores. MLFlow provides the experiment tracking infrastructure to store, compare, and regress-test these scores over time.
